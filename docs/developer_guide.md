...
* [`archivey.api`](reference/archivey/api/index.html) (public API functions, types and classes)
* [`archivey.internal`](reference/archivey/internal/index.html) (base classes and helpers)
* [`archivey.formats`](reference/archivey/formats/index.html) (format-specific readers)

The library exposes an [`ArchiveReader`][archive_reader] abstract base class for users with the public API. The actual format readers extend from the helper class [`BaseArchiveReader`][base_archive_reader], which implements most of the public API by delegating to some simpler methods that the readers must implement. New readers will almost always want to inherit from `BaseArchiveReader`.

## Registering Your Reader

For your reader to be called, you'll need to:

*   add the format(s) your reader handles to [archivey.api.ArchiveFormat](reference/archivey/api/types/index.html#ArchiveFormat);
*   detect archives of these formats by signature and/or filename, in [format_detection.py](reference/archivey/formats/format_detection.html);
*   create your reader class, see below;
*   modify [`archivey.api.core.open_archive`][open_archive] to associate the archive format with your reader.


## Creating a new format reader

It's probably easiest to understand what to do by looking at existing reader implementations. [ZipReader](reference/archivey/formats/zip_reader.html) may be a good starting point.
...
Should accept the same parameters as other `ArchiveReader`s, so it can be called from [`open_archive`][open_archive]:
...
The constructor should call the [`super().__init__(...)`](reference/archivey/internal/base_reader/index.html#BaseArchiveReader.__init__) constructor with the same parameters, plus another one:
...
This method is called by `BaseArchiveReader` to discover and register all members in the archive. It should be implemented to yield [`archivey.api.types.ArchiveMember`](reference/archivey/api/types/index.html#ArchiveMember) objects one by one, from the members in the archive. It will be iterated through only once as the members are needed, so you can use an iterator from the inner library if available.
...
Return an [`archivey.api.types.ArchiveInfo`](reference/archivey/api/types/index.html#ArchiveInfo) object with all archive-level metadata:
...
*   **`_extract_pending_files(self, path: str, extraction_helper: ExtractionHelper, pwd: bytes | str | None)`**:
    ...
    *   Your implementation should read pending extractions via `extraction_helper.get_pending_extractions()`, extract the files and then call `extraction_helper.process_file_extracted(member, extracted_file_path)`. See the implementation in [`SevenZipReader`](reference/archivey/formats/sevenzip_reader.html) for an example.
...
Libraries often have their own exception base classes, or raise builtin exceptions such as `OSError` when there's a problem with an archive. Archivey tries to guarantee that all exceptions raised due to archive issues are subclasses of [`archivey.api.exceptions.ArchiveError`](reference/archivey/api/exceptions/index.html#ArchiveError), and so readers need to translate all exceptions raised by the libraries into them.
...
When opening an archive in your reader `__init__()`, an exception may also be raised. You may use the same translation function by wrapping your archive opening with [`run_with_exception_translation`](reference/archivey/internal/io_helpers.html#run_with_exception_translation).
...
[base_archive_reader]: reference/archivey/internal/base_reader/index.html#BaseArchiveReader
