...
from archivey.exceptions import ArchiveyError, CorruptedArchiveError, MemberNotFoundError
...
class FolderReader(ArchiveReader):
    ...
    def _convert_entry_to_member(self, entry_path: Path, root_path: Path) -> ArchiveMember:
        ...
        try:
            ...
            stat_result = entry_path.stat() if not entry_path.is_symlink() else lstat_result
    ...
    def iter_members(self) -> Iterator[ArchiveMember]:
        ...
        try:
            root_stat = self.archive_path.lstat() # lstat for consistency, though for root it's likely not a symlink itself
            yield ArchiveMember(
             ...
             raise CorruptedArchiveError(f"Cannot stat root directory {self.archive_path}: {e}") from e


        for root, dirs, files in os.walk(self.archive_path, topdown=True, followlinks=False):
    ...
    def open(self, member: ArchiveMember | str, *, pwd: Optional[str | bytes] = None) -> IO[bytes]:
        ...
        if not full_path.exists():
            raise MemberNotFoundError(f"Member not found: {member_name} (resolved to {full_path})")
        ...
        try:
            ...
            if self.archive_path not in resolved_full_path.parents and resolved_full_path != self.archive_path :
                ...
                if not str(resolved_full_path).startswith(str(self.archive_path)):
                    raise MemberNotFoundError(f"Access to member '{member_name}' outside archive root is denied.")

        except OSError as e: # e.g. broken symlink during resolve()
             raise MemberNotFoundError(f"Error resolving path for member '{member_name}': {e}") from e


        try:
            return open(full_path, "rb")
        except OSError as e:
            raise CorruptedArchiveError(f"Cannot open member '{member_name}': {e}") from e
    ...
    def __init_subclass__(cls, **kwargs: Any) -> None:
        ...
        if not hasattr(cls, 'format') or cls.format != ArchiveFormat.FOLDER:
            raise TypeError("FolderReader subclasses must have format set to ArchiveFormat.FOLDER")
