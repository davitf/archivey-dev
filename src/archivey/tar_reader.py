...
from archivey.base import (
    ArchiveReader, ArchiveMember, ArchiveCorruptedError,
    ArchiveMemberNotFoundError, ArchiveInfo, MemberType
)
...
class TarReader(ArchiveReader):
    ...
    def __init__(self, archive_path: str):
        ...
        try:
            ...
            raise ArchiveCorruptedError(f"Invalid compressed TAR archive {archive_path}: {e}")
        ...
        if self._members is None:
            ...
            for info in self._archive.getmembers():
                ...
                member = ArchiveMember(
                    ...
                    type=(MemberType.FILE if info.isfile() else
                          MemberType.DIR if info.isdir() else
                          MemberType.LINK if info.issym() or info.islnk() else
                          MemberType.OTHER),
    ...
    def open(self, member: Union[str, ArchiveMember]) -> io.IOBase:
        if isinstance(member, str):
            try:
                ...
                raise ArchiveMemberNotFoundError(f"Member {member} not found in archive")
        else:
            try:
    ...
    def get_archive_info(self) -> ArchiveInfo:
        ...
        if self._format_info is None:
            ...
            self._format_info = ArchiveInfo(
                format=format,
                is_solid=format != CompressionFormat.TAR,  # True for all compressed TAR formats
                extra={
                    "format_version": self._archive.format if hasattr(self._archive, "format") else None,
                    "encoding": self._archive.encoding if hasattr(self._archive, "encoding") else None
                }
            )
