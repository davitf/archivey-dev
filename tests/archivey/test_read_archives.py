...
def test_read_archive(sample_archive: ArchiveInfo):
    ...
    allow_timestamp_rounding_error = sample_archive.generation_method == GenerationMethod.ZIPFILE
    ...
    with ArchiveStream(full_path_to_archive(sample_archive.filename)) as archive:
        ...
        assert normalize_newlines(format_info.comment) == normalize_newlines(sample_archive.archive_comment)
        ...
        for member in archive.info_iter():
            ...
            if sample_file is None:
                if member.type == MemberType.DIR:
                    logging.warning(f"Archive {sample_archive.filename} contains unexpected dir {member.filename}")
                    continue
                else:
                    pytest.fail(f"Archive {sample_archive.filename} contains unexpected file {member.filename}")
            ...
            if allow_timestamp_rounding_error:
                assert abs(member.mtime.timestamp() - sample_file.mtime.timestamp()) <= 1, f"Timestamp mismatch for {member.filename}: {member.mtime} != {sample_file.mtime}"
            else:
                assert member.mtime == sample_file.mtime, f"Timestamp mismatch for {member.filename}: {member.mtime} != {sample_file.mtime}"

            assert member.encrypted == (sample_file.password is not None)
            password = (sample_file.password or "").encode("utf-8") if member.encrypted else None
